...
Jobs contain `id`, `filename`, `state`, `model`, `language`, `detected_language`, `text`, `segments`, `duration_seconds`, `error`, `created_at`, `updated_at`, `origin`. Segments contain `start`, `end`, `text`; times are seconds. Dates are UTC ISO 8601. States are `queued`, `transcribing`, `completed`, `failed`. Poll an individual job until terminal. Empty completed text means no speech was detected.

Uploads and staged finalization accept `origin=app`, `shared`, `keyboard`, or `unknown` (default). Origin is fixed when a job is created and survives idempotent retries. Existing jobs may omit it. Only completed `app` and `shared` jobs are eligible for transcript notifications, under the device's existing notification preference and foreground rules. Keyboard and unknown jobs are silent.
