...
class ConfigurationTests(unittest.TestCase):
    def test_authentication_is_required_by_default(self):
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(ValueError, "VERSE_DEVICE_SECRET is required"):
                ServerConfig.environment()

    def test_unauthenticated_mode_is_explicit(self):
        with patch.dict(os.environ, {"VERSE_ALLOW_UNAUTHENTICATED": "1"}, clear=True):
            self.assertTrue(ServerConfig.environment().allow_unauthenticated)

    def test_short_device_secret_is_rejected(self):
        with patch.dict(os.environ, {"VERSE_DEVICE_SECRET": "too-short"}, clear=True):
    ...
    def test_placeholder_device_secret_is_rejected(self):
        with patch.dict(
            os.environ,
            {"VERSE_DEVICE_SECRET": "replace-with-a-long-random-device-secret"},
            clear=True,
        ):
    ...
    def test_environment_file_is_loaded_without_overwriting_process_values(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                "VERSE_DEVICE_SECRET=file-secret-with-enough-characters\nVERSE_PORT=9999\n",
                encoding="utf-8",
            )
            with patch.dict(os.environ, {"VERSE_PORT": "8787"}, clear=True):
                load_environment(path)
                self.assertEqual(os.environ["VERSE_DEVICE_SECRET"], "file-secret-with-enough-characters")
                self.assertEqual(os.environ["VERSE_PORT"], "8787")
