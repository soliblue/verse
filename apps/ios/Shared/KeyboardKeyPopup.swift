...
final class KeyboardKeyPopup: UIView {
    ...
    func update(text: String, keyFrame: CGRect, keyboardWidth: CGFloat, dark: Bool) {
        ...
        path.addLine(to: CGPoint(x: right, y: bottom - 8))
        path.addQuadCurve(to: CGPoint(x: right - 8, y: bottom), controlPoint: CGPoint(x: right, y: bottom))
        path.addLine(to: CGPoint(x: left + 8, y: bottom))
        path.addQuadCurve(to: CGPoint(x: left, y: bottom - 8), controlPoint: CGPoint(x: left, y: bottom))
