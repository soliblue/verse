...
enum KeyboardKeyLayout {
    ...
    static func keys(in bounds: CGRect, mode: Mode, globe: Bool) -> [Key] {
        ...
        let contentWidth = bounds.width - 10
        ...
        let modifierWidth = max(40, width * 1.28)
        ...
        let modeWidth = contentWidth * (globe ? 0.135 : 0.16)
        let globeWidth = globe ? contentWidth * 0.135 : 0
        let returnWidth = contentWidth * 0.245
        ...
        return result.map { Key(slot: $0.slot, action: $0.action, frame: $0.frame.offsetBy(dx: 5, dy: 0)) }
    }
}
#endif
