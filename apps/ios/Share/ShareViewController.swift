...
final class ShareViewController: UIViewController {
    ...
    private func loadAttachment() {
        ...
        provider.loadFileRepresentation(forTypeIdentifier: type) { [weak self] url, error in
            guard let url, error == nil,
                ...
                DispatchQueue.main.async { self?.status.text = "Could not read this file. Choose audio under 50 MB." }
                return
            }
            ...
            guard (try? FileManager.default.copyItem(at: url, to: temporary)) != nil else {
                DispatchQueue.main.async { self?.status.text = "Could not prepare this file. Try sharing it again." }
                return
            }
            DispatchQueue.main.async { self?.upload(temporary, filename: filename) }
        }
    }
    ...
    private func upload(_ file: URL, filename: String) {
        ...
        URLSession.shared.uploadTask(with: request, fromFile: file) { [weak self] data, response, error in
            ...
            guard error == nil, let response = response as? HTTPURLResponse,
                ...
                DispatchQueue.main.async { self?.status.text = "Upload failed. Check your connection and token in Verse." }
                return
            }
            VerseBridge.pendingJobID = job.id
            DispatchQueue.main.async { self?.poll(job.id) }
        }.resume()
    }
